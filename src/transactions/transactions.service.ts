...
import {FindTransactionsOptions, PaginationQuery} from "../def/pagination-query";
...
export class TransactionsService {
    ...
    private async cancelParkingPayment(sdkOrderId: string, reason: string) {
        try {
            ...
            throw new InternalServerErrorException('Could not cancel payment at this time.');
        }
    }
    ...
    private async finalizePayment(transactionId: string) {
        ...
        if (transaction.parkingSession) {
            await this.parkingSessionRepository.update(transaction.parkingSession.id, {status: ParkingSessionStatus.COMPLETED});
        }
    }
    ...
    async findAll(query: PaginationQuery, status?: string) {
        const relations = ['parkingSession', 'parkingSession.spot', 'parkingSession.spot.lot'];
    ...
    async createParkingTransaction(payload: {
        ...
        const session = await this.parkingSessionRepository.findOne({where: {id: payload.sessionId}});
        ...
        const response = await this.pokApiService.createTransaction(transactionPayload);
    ...
    async prepare3DS(sdkOrderId: string, cardId: string, userId: string) {
        return await this.cardsService.setupTokenized3DS(cardId, sdkOrderId, userId);
    }
    ...
    async get3DSConfiguration(sdkOrderId: string, cardId: string) {
        ...
        return await this.pokApiService.setupTokenized3DS(card.pokCardId, sdkOrderId);
    }
