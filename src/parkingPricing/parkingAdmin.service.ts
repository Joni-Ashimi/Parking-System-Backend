...
export class ParkingAdminService {
    constructor(
        ...
        private parkingSpotTypeRepository: Repository<ParkingSpotType>,
    ...
    async getAllTypes() {
        return this.parkingSpotTypeRepository.find({
    ...
    async updateBasePricing(typeId: string, data: {
        ...
        const type = await this.parkingSpotTypeRepository.findOneBy({ id: typeId });
        if (!type) throw new NotFoundException('Type not found');

        type.baseHourlyRate = data.baseHourlyRate;
        type.baseDailyRate = data.baseDailyRate;

        return this.parkingSpotTypeRepository.save(type);
    }
