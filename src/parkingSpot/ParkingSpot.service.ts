...
import { ParkingSpotTypeCode } from 'src/def/enums/ParkingSpotType';
...
export class ParkingSpotService {
  ...
  async create(dto: CreateParkingSpotDto): Promise<ParkingSpot> {
    ...
    if (!targetRow || parts.length < 2) {
      throw new BadRequestException(
        'Invalid spot number formatting sequence. Expected format like "A-01".',
      );
    }
    ...
    if (!spotCategory) {
      throw new NotFoundException(
        `SpotCategory with id "${dto.typeId}" not found.`,
      );
    }
  ...
  async findAll(queryDto: GetParkingSpotsQueryDto = {}) {
    ...
    query.orderBy('spot.floor', 'ASC').addOrderBy('spot.spotNumber', 'ASC');
    ...
    const processedData = data.map((spot) => {
      ...
      const activeRule = spot.type.rules?.find((rule) => {
        ...
        const ruleStart =
          rule.startHour !== null ? Number(rule.startHour) : null;
      ...
      return {
        ...
        type: {
          ...
          effectiveHourlyRate: Number(
            Math.max(0, effectiveHourlyRate).toFixed(2),
          ),
    ...
    return spots.map((spot) => {
      ...
      if (type?.rules?.length) {
        const activeRule = type.rules.find((rule) => {
          const matchesDay =
            rule.dayOfWeek === null || rule.dayOfWeek === now.getDay();
          const matchesHour =
            rule.startHour === null ||
  ...
  async findAllAvailable(
    lotId?: string,
    typeCode?: ParkingSpotTypeCode | undefined,
  ): Promise<ParkingSpot[]> {
    const query = this.parkingSpotRepository
      .createQueryBuilder('spot')
      .leftJoinAndSelect('spot.lot', 'lot')
  ...
  async update(id: string, dto: UpdateParkingSpotDto): Promise<ParkingSpot> {
    ...
    if (dto.spotNumber !== undefined) {
      ...
      if (existingSpot && existingSpot.id !== id) {
        throw new ConflictException(
          `Parking spot number "${dto.spotNumber}" already exists.`,
        );
      }
  ...
  async remove(id: string): Promise<void> {
    ...
    if (spot.status === ParkingSpotStatus.OCCUPIED) {
      throw new ConflictException('Cannot delete an occupied parking spot');
    }
